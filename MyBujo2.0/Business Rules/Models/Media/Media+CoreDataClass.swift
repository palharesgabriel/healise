...
public class Media: NSManagedObject {
	...
	var photos: [UIImage]? {
        guard let paths = getPaths(type: .photo) else { return nil }
    ...
    private func getPaths(type: DirectoryType) -> [String]? {
		do {
            let paths = try FileManager.default.contentsOfDirectory(atPath: self.mainPath.appendingPathComponent(type.rawValue).path)
			return paths
		} catch {
			print(error)
			return nil
